...
pub trait ConfigApi {
    fn dubbo_protocol_set(&self, protocol: &str, pairs: Vec<(&str, &str)>) -> Result<(), Error>;
    ...
    fn dubbo_registry_set(&self, registry_id: &str, pairs: Vec<(&str, &str)>) -> Result<(), Error>;
    fn dubbo_services_name(&self) -> Option<Vec<String>>;
    fn dubbo_services_get(&self, service_name: &str) -> Result<Service, Error>;
    fn dubbo_services_set(&self, service_name: &str, pairs: Vec<(&str, &str)>)
        -> Result<(), Error>;
    ...
    fn dubbo_protocol_set(&self, protocol: &str, pairs: Vec<(&str, &str)>) -> Result<(), Error> {
        ...
        for pair in pairs {
            ...
            match pair.0 {
                ...
                "name" => x.name = value,
                _ => {
                    HashMap::insert(&mut x.params, pair.0.to_string(), value);
                }
            }
        }
    ...
    fn dubbo_registry_set(&self, registry_id: &str, pairs: Vec<(&str, &str)>) -> Result<(), Error> {
        ...
        for pair in pairs {
            ...
            match pair.0 {
                ...
                "registry_type" => {
                    x.registry_type = value.split(",").map(|x| x.to_string()).collect()
                }
    ...
    fn dubbo_consumer_references_set(
        ...
        for pair in pairs {
            ...
            match pair.0 {
                ...
                "url" => x.url = value,
                "registry_ids" => {
                    x.registry_ids = value.split(',').map(|x| x.to_string()).collect()
                }
                _ => {
                    return Err(anyhow!(ConfigError::UnsupportedKey(
